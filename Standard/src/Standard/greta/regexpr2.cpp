...
template< typename IterT, typename SyntaxT >
REGEXPR_H_INLINE void STANDARD_API basic_rpattern_base<IterT, SyntaxT>::_common_init( REGEX_FLAGS flags )
{
    this->m_cgroups = 0;
    ...
    SyntaxT & sy )
{
    ...
    if( iend != itemp && CHRSET_NEGATE == sy.charset_token( itemp, iend ) )
    {
    ...
    do
    {
        ...
        if( CHRSET_RANGE == tok && fhave_prev )
        {
    ...
    while( check_iter<iter_type>( iprev = icur, iend ),
           CHRSET_END != ( tok = sy.charset_token( icur, iend ) ) );
